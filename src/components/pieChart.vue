...
export default {
  ...
  methods: {
    clearChart: function () {
      var can = document.getElementById('pie_chart')
    ...
    drawPieChart: function (info) {
      ...
      var ctx = can.getContext('2d')
      var width = innerWidth * 0.7 // 饼状图的宽
      var height = innerHeight * 0.5 // 饼状图的高
      ...
      CanvasRenderingContext2D.prototype.mark = function (x, y, radius, start, end, position, num) {
        var rad = (start + end) / 2
        ...
        this.fillStyle = 'black'
        this.font = '16px scans-serif'
        this.fillText(num + '%', position[0] + 5, position[1] + 5)
      }
      ...
      for (m = 0; m < rad.length - 1; m++) {
        ...
        ctx.mark(ox, oy, radius, rad[m], rad[m + 1], position, 20)
      }
    }
  }
}
